...
| 管理画面へのアクセス拒否の件数 | 監査ログの ACCESS_DENIED の件数 | 1日で 50 件超（見積もり: 1日数十件以内。U3 の NFR1.5） | 運用で見る指標の候補（U3 の NFR10.5）。本Intentでは監査ログから数え、指標としては作らない |
...
|---|---|---|---|
| 管理画面への拒否の増加 | 403 の件数が 1 時間に 20 件超、または ACCESS_DENIED が 1 日で 50 件超 | 中 | 当面: 開発者が確認（監査ログで利用者・接続元IP を確かめる）。配備先が決まったら通知の先を決める |
