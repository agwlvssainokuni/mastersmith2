...
| 取り消し | 1章のテスト |
| 要求のパス | ACCESS_DENIED に正規化済みの要求のパスが問い合わせの部分なしで記録され、ログインの出来事では空であること |
| 切り詰め | 300 文字のメールアドレス、600 文字の要求のパス、サロゲートペアの途中で切れる User-Agent（性質ベースのテスト（jqwik）で、切り詰めの結果が壊れた文字を含まず上限以下であることを確かめる） |
