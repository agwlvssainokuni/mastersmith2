...
4. 出来事から監査イベントを作る。種類から結果を決め（BR1.2）、日時は出来事の日時とする（BR1.5）。メールアドレスと User-Agent は、文字（コードポイント）単位で数えて長さを超える分を、文字を分断しない位置で切り詰める（BR2.1）。存在しないメールアドレスもそのまま記録する（BR1.6）。
5. 内部DBに追記する（BR4.1）。
6. 書き込みに失敗したら、WF3 へ。
...
| User-Agent の 512 文字目がサロゲートペア（絵文字など）の途中 | その文字の手前で切り、壊れた文字を残さない | BR2.1 |
| Cookie が無い・無効な状態でログアウト | U2 が出来事を作らないため、記録されない（U2 の決まり 6.1） | BR1.1 |
| 元の操作の確定後、監査イベントを書く前にアプリが停止 | その1件は失われる（受け入れる） | BR3.1 |
