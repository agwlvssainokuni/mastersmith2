...
|---|---|---|---|
| ログインの API の応答時間 | `http.server.requests`（`/api/auth/login` の 95 パーセンタイル） | 1 秒超 | U2 の NFR1.1 |
...
| ログインの失敗の割合 | 監査ログの LOGIN_FAILED ÷ ログインの試行 | 1日で 30% 超 | 本Intentでは監査ログから数える（U2 の NFR10.7） |
...
|---|---|---|---|
| ログインの遅れ | ログインの API の 95 パーセンタイルが 1 秒超 | 中 | 当面: 開発者が確認。配備先が決まったら通知の先を決める |
| 照合の時間の範囲外 | 起動時に照合の時間の WARN が出る | 低 | 同上（cost か CPU の上限を見直す） |
...
|---|---|---|
| ログインの API の応答時間（95 パーセンタイル） | 1 秒以内（同時 10 件） | Performance Validation の試験の間、配備先では 30 日 |
