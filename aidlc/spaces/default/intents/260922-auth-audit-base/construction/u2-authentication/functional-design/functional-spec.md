...
3. 設定を検査する。メールアドレスかパスワードが無い、メールアドレスの形式が正しくない、パスワードが12文字未満、パスワードが UTF-8 で 72 バイトを超える、のいずれかなら、作らずに WARN を出して終える（例外で起動を止めない）。警告には、足りない・正しくない項目と、設定を直して再起動すると作成されることを載せる。パスワードの値は載せない（BR1.3、BR1.4）。
...
3. 利用者を探す。
   - いない → 利用者と結びつかないダミーの記録に対して、手順 4・5 と同じ読み取りと書き込みを行い（BR2.7）、ダミーの照合を行って（BR2.5）、理由 USER_NOT_FOUND で失敗（手順 7）。そのメールアドレスのロックの状態は作らない（BR3.9）。
4. その利用者のロックの状態を、同じ利用者の他の試みと重ならないように読む（BR3.8）。
   - 解除時刻を過ぎていれば、ロックを解き、失敗回数を0に戻す（BR3.4）。
   - まだロック中なら、値を変えない書き込みを1回行い（BR2.7）、ダミーの照合を行って（BR2.5）、状態の値を変えずに理由 ACCOUNT_LOCKED で失敗（BR3.3、手順 7）。
5. パスワードを照合する。パスワードが UTF-8 で 72 バイトを超える場合は照合の仕組みに渡さず、ダミーの照合で代えて、一致しないものとして扱う（BR2.8）。
...
7. 失敗のときは、LOGIN_FAILED を理由つきで知らせ（BR2.6、BR7.1）、理由によらず同じ 401 / `AUTHENTICATION_FAILED` を返す（BR2.4）。3つの失敗の経路は、内部DBへの読み書きの種類と回数が同じになる（BR2.7）。
...
1. 要求にアクセストークンを付けて送る。
2. 401 / `AUTHENTICATION_REQUIRED` を受けたら、トークンの更新を行う。同じタブで同時に複数の要求が 401 を受けても、更新は1回にまとめる（BR8.5）。ログイン・トークンの更新・ログアウトの API の呼び出しは、この処理の対象外とする（BR8.5）。
...
| 存在しない・誤り・ロック中の3通り | 応答の状態コード・内容が同じ。内部DBへの読み書きの種類と回数も同じ | BR2.4、BR2.7 |
| localhost 以外のホスト名・IP へ http でアクセス | Secure の Cookie が送られず、ログインは働かない（当面の対象外。動作確認は localhost） | BR5.2 |
...
| 初期管理者のパスワードが UTF-8 で 73 バイト | 作らず、理由と直し方を WARN。起動は続く（例外にならない） | BR1.3 |
| ログインで UTF-8 で 73 バイトのパスワード | 例外にならず、パスワード誤りと同じ 401。失敗回数は1増える | BR2.8 |
