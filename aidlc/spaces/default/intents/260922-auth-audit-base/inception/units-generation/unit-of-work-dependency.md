...
| U1 | U2、U3（画面） | 画面の骨組みの差し込み口: 画面（ルート）の登録、サイドバーの項目の登録、ユーザーメニューの項目の登録、ログイン状態の提供元の登録 | 画面内の登録 |
...
Domain Design の部品の依存では、AppFrame（U1）が AuthUi（U2）と AdminArea（U3）に依存しており、単位の依存の向き（U2・U3 が U1 に依存）と逆になる。単位では、U1 が画面の骨組みに差し込み口を用意し、U2・U3 が自分の画面・メニューの項目・ログイン状態の提供元をそこに登録する形にする。これにより、AppFrame は登録されたものを使うだけになり、AuthUi・AdminArea を直接知らない。依存の向きは U2・U3 → U1 にそろい、U1 は U2・U3 が無くても（未ログインとして）動く。U2・U3 は U1 のファイルを書き換えない。これは、ApiClient が AuthUi に直接依存せず、トークンの手段を登録してもらう形（Domain Design の ADR-007）と同じ考え方である。差し込み口の具体的な形は U1 の Contract Design で決める。
...
なし。確定回答 Q4 により4単位を一列に並べたため、互いに依存しない単位の組み合わせは存在しない。単位を作る順番は Delivery Planning で決める。
