...
| 検査 | 道具とタスク | 統合を止める基準 | 実測（2026-09-24、2回の `verify`） |
|---|---|---|---|
| 秘密情報の検出 | Gitleaks（`gitleaksScan`、履歴全体）。pre-commit でも実行 | 検出 1件以上 | 141 コミット・約 15.65MB を走査して 0 件 |
...
| 依存関係の脆弱性 | OSV-Scanner（`osvScan`、`build/reports/osv-scanner/osv.json`） | 実行時の依存の High 以上、`MAL-` で始まるもの、`config/npm-build-tools.txt` の道具の High 以上（README の判定の表） | 2回とも UP-TO-DATE（lockfile が変わっていないため、同じ日の 13:34 の実行の結果を使った）。その結果は該当 0 件 |
...
- OSV-Scanner は依存が変わらない限り UP-TO-DATE になる。新しい脆弱性のデータを反映させたいときは `./gradlew osvScan --rerun-tasks` を実行する。
- CodeQL などのより広い静的解析、起動したアプリへの動的検査（OWASP ZAP）、コンテナイメージの検査は採用しない（team.md の Code Style）。

## 2. 振る舞いを確かめるテスト（既に生成されているもの）

すべて `./gradlew verify` の中で実行され、2回の実行とも成功した（`test-results.md` 2.2）。表の「クラス」は `backend/src/test/java/cherry/mastersmith/` または `frontend/src/` からの相対。
...
| コンテナの実行環境が無いときの動き（NFR12.3） | 警告と SKIPPED は出るが、4クラスが失敗になる不具合がある（Not Met。`test-results.md` 3.1）。安全の抜け穴ではないが、「黙って飛ばさない・止めない」の決まりに関わる |
...
- OSV-Scanner はこの段の2回の `verify` では UP-TO-DATE で、実際に走査したのは同じ日の 13:34 の実行（lockfile は同じ）である。脆弱性のデータベースの更新をこの段で反映させたい場合は `./gradlew osvScan --rerun-tasks` の実行が要る。
